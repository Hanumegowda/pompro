...
public class HappyDayImageManagementToolsQCSandbox extends TestNG_UI_EXTENSIONSForOneBrowser{
	...
	**************************************************************************************/
	...
	public void testHappyDay167455() throws Exception {
		...
		/*
		 * objViewPatients.clickOnSearch().clickOnSearch()
		 * .enterTextInFind(objViewPatients_data.strStudy1) .clickOnSearch();
		 * try { objViewPatients.clickOnReleaseStudyFromQc();
		 * Thread.sleep(5000); Set<String> aw = driver.getWindowHandles();
		 * Iterator<String> itr1 = aw.iterator(); while (itr1.hasNext()) {
		 * String cw = itr1.next().toString(); System.out.println(" a"); if
		 * (!cw.contains(mainwindow)) { System.out.println(" b");
		 * driver.switchTo().window(cw); driver.manage().window().maximize();
		 * objViewPatients.clickOnOkInReleaseStudyFromQcWindow()
		 * .clickOnCloseInMergeReportsPage(); } }
		 * driver.switchTo().window(mainwindow); } catch (NoSuchElementException
		 * ne) { System.out.println("element not present"); }
		 */
		objViewPatients.selStatusOption(objViewPatients_data.ststusAny)
				.enterTextInFind(objViewPatients_data.strStudy1)
				.clickOnSearch().clickOnSearch()
