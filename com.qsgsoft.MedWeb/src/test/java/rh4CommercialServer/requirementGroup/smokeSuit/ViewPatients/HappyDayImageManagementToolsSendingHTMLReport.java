...
public class HappyDayImageManagementToolsSendingHTMLReport extends TestNG_UI_EXTENSIONSForOneBrowser{
  public HappyDayImageManagementToolsSendingHTMLReport() throws Exception {
	  super();
  }
  
  /*************************************************************************************
  'Description   : Verify that a study can be sent to an e-mail recipient when mandatory and 
                   non-mandatory (all) fields are filled in ‘Send study to recipient as HTML 
                   report by E-Mail’ page.     
  'Precondition  :
  'Date          : 24-Sep-2015
  'Author        : Hanumegowda
  '---------------------------------------------------------------
  ...
  'Date                                                         Name
	**************************************************************************************/
	@Test(description = "Verify that a study can be sent to an e-mail recipient when mandatory "
			+ "and non-mandatory (all) fields are filled in ‘Send study to recipient as HTML report by E-Mail’ page.")
	public void testHappyDay68462() throws Exception {

		gstrTCID = "68462";
		gstrTO = "Verify that a study can be sent to an e-mail recipient when mandatory and non-mandatory (all) fields"
				+ " are filled in ‘Send study to recipient as HTML report by E-Mail’ page .";
		...
		while (itr.hasNext()) {
			...
			if (!childwindow.contains(mainwindow)) {
				...
				objViewPatients.selRecipientHTMLReportEMailRadioButton()
						.clickOnNextInSendStudyWizard()
						.enterTextInToField(objViewPatients_data.strUN)
						.enterTextInSubField(objViewPatients_data.subname)
		...
		.navigateToInbox1(objViewPatients_data.subname).verPdfFile(objViewPatients_data.subname);
		/*objMailVerificationFunctions
				.loginToMail(objViewPatients_data.strUN,
						objViewPatients_data.strPWD)
				.navigateToInbox(objViewPatients_data.subname).verHtmlFile();*/
