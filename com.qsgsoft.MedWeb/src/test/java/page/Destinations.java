...
public class Destinations extends PageObject{
	...
	**********************************************************************************/
	...
	/**********************************************************************************
	* Description : This function is to delete required destination.
	* Date 		  : 05-Aug-2015
	* Author 	  : Hanumegowda
	**********************************************************************************/
	public Destinations DelDestination(String dest) throws Exception {
		objFrames.switchToDefaultFrame().switchToRightFrame()
		.switchToMainInRightFrame();
		String str = "//font[contains(text(),'"+dest+"')]/../../following-sibling::td//a/img[@src='/cobalt-images/delete.gif']";
		objWaitForElement.waitForElements(str, "xpath");
		List<WebElement> ae = this.page.element(str, "xpath").get();
