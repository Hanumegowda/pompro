...
public class HappyDayCreateOrder extends TestNG_UI_EXTENSIONSForOneBrowser{
	...
	**************************************************************************************/
	...
	public void testHappyDay62733() throws Exception {
		...
		objViewPatients.enterTextInFind(str2).clickOnSearch().clickOnSearch()
				.clickOnSearch().verStatStatusWithRedCircle(str2).clickOnLogout();
    ...
    'Date                                                         Name
	**************************************************************************************/
	@Test(description = "Verify that when the order is created with the procedure length, the order"
			+ " is displayed in Scheduler page with the above procedure time.")
	...
	**************************************************************************************/
	...
	public void testHappyDay62731() throws Exception {
		...
		objRISSettings.clickOnOpenPageOfInsuranceCompanies().clickOnAdd()
				...
				.clickOnSaveChanges();
		objRISSettings.clickOnClinicProcedure1();
		...
		objPatientScheduleRegistration.clickOnSearchIconOfReasonForStudy();
		Set<String> awa111 = driver.getWindowHandles();
		Iterator<String> itra111 = awa111.iterator();
		while (itra111.hasNext()) {
			String cw = itra111.next().toString();
			if (!cw.contains(mw1)) {
				driver.switchTo().window(cw);
				driver.manage().window().maximize();
				objPatientScheduleRegistration
						.clickOnAddIconInReasonForStudyPage()
						.enterReasonInReasonForStudyPage(
								objRISSettings_data.strReason);
				objDicomRemapping.clickOnSave();
				objPatientScheduleRegistration
						.selDoctorOrRefPhy(objRISSettings_data.strReason);
				driver.switchTo().window(mw1);
			}
		}
