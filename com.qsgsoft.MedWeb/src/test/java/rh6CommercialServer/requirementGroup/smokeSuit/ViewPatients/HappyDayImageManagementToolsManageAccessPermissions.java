...
public class HappyDayImageManagementToolsManageAccessPermissions extends
		TestNG_UI_EXTENSIONSForOneBrowser {
	public HappyDayImageManagementToolsManageAccessPermissions()
			throws Exception {
		super();
	}

	/*************************************************************************************
    'Description   : Verify that when a study is assigned to more than one user, all the assignees
                     are able to view the study on logon to the server.     
    'Precondition  :
    'Date          : 23-Sep-2015
    'Author        : Hanumegowda
    '---------------------------------------------------------------
    ...
    'Date                                                         Name
	**************************************************************************************/
	@Test(description = "Verify that when a study is assigned to more than one user, all the "
			+ "assignees are able to view the study on logon to the server.")
	public void testHappyDay68451() throws Exception {

		gstrTCID = "68451";
		gstrTO = "Verify that when a study is assigned to more than one user, all the assignees "
				+ "are able to view the study on logon to the server.";
		...
		objGroups.verAddNewGroup().enterGroupName(objGroups_data.groupname)
				...
				.clickOnCopy().selectUserFilterstoRestrictCheckbox();
		...
		objViewPatients.clickOnSearch1().clickOnAccessPermissionKey(
		...
		while (itr.hasNext()) {
			...
			if (!childwindow.contains(mainwindow)) {
				...
				objCommonFunctions.clickOnContinueToThisWebsiteLink();
				objViewPatients.selUserCheckBox(objUser_data.strNewUserName)
						.selUserCheckBox(objUser_data.strNewUserName1)
						.clickOnChange();
			}
		}
