package rh4CommercialServer.requirementGroup.smokeSuit.AddendumReports;
	...
	**************************************************************************************/
	...
	public void testHappyDay158046() throws Exception {
		...
		objViewPatients.enterTextInFind(objViewPatients_data.strStudy)
				...
				.clickOnSearch();
		objViewPatients
				.clickOnViewModifyWebReportsOfAStudyAsOtherUser(objViewPatients_data.strStudy);
		String dtousr = "";
	...
	public void testHappyDay168532() throws Exception {
		...
		while (ite1.hasNext()) {
			...
			if (!childwindow.contains(mainwindowhandle1)) {
				...
				objViewPatients.verWebReporterPage("Web-Reporter")
						.clickOnNewAddEndumIcon();
				Set<String> allwindows = driver.getWindowHandles();
				Iterator<String> ite11 = allwindows.iterator();
				while (ite11.hasNext()) {
					String childwindow1 = ite11.next().toString();
					if ((!childwindow1.contains(childwindow))
							&& (!childwindow1.contains(mainwindowhandle1))) {
						driver.switchTo().window(childwindow1);
						objViewPatients
								.enterTextInReferringPhysicianFieldInAddendumReportEditorPage(
										objViewPatients_data.strRefPhyConstant)
