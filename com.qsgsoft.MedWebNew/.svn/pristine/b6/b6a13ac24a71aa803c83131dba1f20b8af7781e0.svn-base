...
public class HappyDayUserFilter extends TestNG_UI_EXTENSIONSForOneBrowser {
	...
	public void testHappyDay107527() throws Exception {
		...
		while (itr.hasNext()) {
			...
			if (!mainwindow.contains(childwindow)) {
				driver.switchTo().window(childwindow);
				strmtf = objAddUser.selAPhysicianAndGetHisName();
				Thread.sleep(5000);
		...
		objLogin.loginAsUser(objUser_data.strNewUserName,
				objUser_data.strNewPassword).clickLogin();
		objViewPatients.clickOnSearch().clickOnSearch().clickOnSearch();
		objViewPatients.verRefferingPhysicianName(strmtf.replace(".", " "));
	...
	**************************************************************************************//*
	...
	public void testHappyDay83928() throws Exception {
		...
		gstrTCID = "83928";
		gstrTO = "Verify that the user is allowed to view studies from the user specified in "
				+ "'By Consulting' field entered while creating users.";
		...
		objUserManagement.clickOnAddUser();
		objAddUser.verAddNewUserPage()
				.selectOptionInACLgroupDropDown(objUser_data.generalist)
				.enterFullNameLast(objUser_data.strgenUserName)
				.enterPassWord(objUser_data.strNewPassword)
				.confirmPassWord(objUser_data.strNewPassword)
				.clickOnSaveChanges();
		System.out.println(objUser_data.strgenUserName);
		objHome.launchApplication();
		objFrames.switchToDefaultFrame().switchToMainMenuInLeftFrame();
		objHome.clickOnViewPatients();
		objFrames.switchToDefaultFrame().switchToRightFrame()
				.switchToMainInRightFrame();
		objLogin.enterUserName(objUser_data.strtelUserName)
				.enterPassWord(objUser_data.strNewPassword).clickLogin();
		objViewPatients.clickOnGeneral();
		...
		objViewPatients.selConsultingMD(objUser_data.strgenUserName)
				.enterLastName(objUser_data.strLastName)
				.enterFirstName(objUser_data.strFirstName).selDOBDay()
				.selDOBMonth().selDOBYear().clickOnDataFiles().clickOnSubmit()
				.clickOnLogOutInTelService();
		...
		objGroups.verAddNewGroup().enterGroupName(objGroups_data.groupname)
				.enterFullGroupName(objGroups_data.groupname)
				.selectGroupType(objGroups_data.grouptypeDent)
				.selectAllowedtoAccessServerCheckbox()
				.selectUserFilterstoRestrictCheckbox();
		objAddUser.clickOnSaveChanges();
		objFrames.switchToDefaultFrame().switchToLeftFrame()
				.switchToMainMenuInLeftFrame();
		objAdministratorSite.clickOnUser();
		...
		for (String element : windowHandels) {
			if (!element.equals(mainWindowHandle)) {
				driver.switchTo().window(element);
			}
		...
		objAddUser.verSelectConsultingPhysiciansWindow()
				.selConsultingPhysician(objUser_data.strgenUserName);
		...
		objViewPatients.verifyUser(objUser_data.strNewUserName)
				.clickOnSearch()
				.verConsultingPhysicianName(objUser_data.strgenUserName);
		gstrResult = "PASS";
...
*/
	...
	**************************************************************************************/
	...
	/*************************************************************************************
    'Description   : Verify that the user is allowed to view studies from the user specified 
                     in 'By Consulting' field entered while creating users.      
    'Precondition  :
    'Date          : 26-Oct-2015
    'Author        : Hanumegowda
    '---------------------------------------------------------------
    ...
    'Date                                                         Name
	**************************************************************************************/
	@Test(description = "Verify that the user is allowed to view studies from the user specified"
			+ " in 'By Consulting' field entered while creating users.")
	public void testHappyDay839281() throws Exception {

		gstrTCID = "83928";
		gstrTO = "Verify that the user is allowed to view studies from the user specified in 'By "
				+ "Consulting' field entered while creating users.";
		...
		objLogin.loginAsUser(objUser_data.strNewUserName,
				objUser_data.strNewPassword).clickLogin();
		objViewPatients.clickOnSearch().clickOnSearch().clickOnSearch();
		objViewPatients.verConsultingPhysicianName(strmtf);
	...
	**************************************************************************************/
	...
	public void testHappyDay83945() throws Exception {
		...
		objLogin.loginAsUser(objUser_data.strNewUserName,
				objUser_data.strNewPassword).clickLogin();
		objViewPatients.selLayout(objViewPatients_data.strLayout).clickOnSearch();
		Thread.sleep(5000);
		...
		objDICOMSetting
				.enterTextInInstitutionNameTextField(
						objDICOMSettings_data.strInstitute1)
				.clickOnSaveChanges().clickOnDestinations();
	...
	**************************************************************************************/
	...
	public void testHappyDay106196() throws Exception {
		...
		objViewPatients.selInLastOption(objDICOMSettings_data.hour)
				...
				.clickOnSearch();
		objViewPatients.verStudyFromOtherServer(str2);
